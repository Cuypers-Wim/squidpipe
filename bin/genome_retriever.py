...
This script processes a CSV file containing taxon IDs and sample names linked to a certain barcode folder 
produced by nanopore sequencing, or takes them as command-line arguments.
It attempts to download the most recent RefSeq reference genome for those taxon IDs. If no RefSeq reference 
can be found, the script will try to obtain the most complete Genbank .fna file. If no genome is found for a 
given taxon ID, it will attempt to download genomes for its parent taxon IDs.

Usage:
    genome_retriever.py -c <path_to_csv_file> -o <output_directory>
    OR
    genome_retriever.py --taxon_id <taxon_id> --name <sample_name> -o <output_directory>
...
def process_csv(csv_file, output_dir):
    """
    Process the CSV file and download genomes.
...
def main(args):
    """
    Main function to handle argument parsing and decide whether to process a CSV file or individual inputs.
    """
    if args.csv_file:
        process_csv(args.csv_file, args.output_dir)
    else:
        if args.taxon_id and args.name:
            log(f"Processing {args.taxon_id} ({args.name})")
            if not process_taxon(args.taxon_id, args.name, args.output_dir):
                parent_taxon_ids = get_parent_taxon_ids(args.taxon_id)
                for parent_id in parent_taxon_ids:
                    log(f"Attempting to download genome for parent taxon ID {parent_id}")
                    if process_taxon(parent_id, args.name, args.output_dir):
                        break
                else:
                    log(f"No .fna file found for any parent taxon ID of {args.taxon_id} ({args.name})")
        else:
            log("Error: If not using a CSV file, you must provide taxon_id, name, and output_dir as arguments.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process a CSV file or individual taxon ID and sample name to download genomes.")
    parser.add_argument('-c', '--csv_file', help="Path to the CSV file containing taxon IDs and sample names.")
    parser.add_argument('-t', '--taxon_id', help="Taxon ID for the sample.")
    parser.add_argument('-n', '--name', help="Name of the sample.")
